...
from groq import AsyncGroq  # 공식 Groq 비동기 라이브러리 도입 (405 에러 방지)
import asyncio
import os
...
async def generate_with_groq(prompt):
    ...
    try:
        # 라이브러리가 주소와 헤더 처리를 내부적으로 완벽하게 제어함
...
@bot.event
async def on_ready():
    print(f'{bot.user} 봇 로그인 성공! (안정적인 Groq SDK 적용 버전)')
